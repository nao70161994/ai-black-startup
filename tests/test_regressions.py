...
const context = {
  ...
  caches: {
    ...
    keys: function () { return Promise.resolve(['ai-black-startup-old', 'other-cache', 'ai-black-startup-2026.05.24.60']); },
...
events.fetch({
  request: { method: 'GET', url: 'https://nao70161994.github.io/main.js?v=20260524-60', mode: 'same-origin' },
...
def test_guided_experience_covers_all_eight_uiux_requirements():
    ...
    assert 'body[data-page="home"] .office-stage { width: 100%; height: 100svh; min-height: 620px; border: 0; border-radius: 0; box-shadow: none; }' in css
    assert 'body[data-page="home"] .command-workspace { padding: 0; }' in css
    assert 'body[data-page="home"] .app-shell { width: 100%; max-width: none; margin: 0; }' in css
    assert '.strategy-option span { color: #b2c0c7; font-size: 10px;' in css
    assert '.log-content p { color: #d3dee3; font-size: 12px;' in css
...
def test_real_browser_ui_experience_checker_covers_pages_states_widths_and_modals():
    ...
    for width in ("320", "390", "768", "960", "1280", "1600"):
    ...
    assert '"edgeGap"' in checker
    assert '"borderRadius"' in checker
    assert '"criticalTextMin"' in checker
    assert 'critical supporting text is below 10px' in checker
