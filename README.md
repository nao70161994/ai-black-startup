...
現在のUIは単一ダッシュボードではなく、目的と空間表現が異なる5ページに分けています。「オフィス」はAI社員が実際の担当場所で働くフルスクリーンの会社フロアで、主要指標・次の指令・リスク・最新ログを空間上のHUDとして重ねます。「製品ラボ」は5工程の製品ライン、「AIクルー」は大きなキャラクターから始める編成、「経営会議」は方針比較・推移・実績・ミッション、「アーカイブ」は時系列ログ・保存・共有・復旧を担当します。960px以上では常設サイドコマンド、959px以下では固定下部ナビを使います。URLは `#home`、`#products`、`#team`、`#management`、`#records` と同期するため、再読込とブラウザの戻る操作でも現在ページを維持します。未判断、製品リスク、未受取報酬はナビの件数バッジでも確認できます。 各ページの補助文は10px以上を基準とし、外側パネルの装飾を抑え、主要CTAと補助操作を明確に区別しています。
